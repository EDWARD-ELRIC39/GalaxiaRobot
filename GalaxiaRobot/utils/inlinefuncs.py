...
from GalaxiaRobot import BOT_USERNAME, DEV_USERS, EVENT_LOGS, arq
from GalaxiaRobot import pbot as app
from GalaxiaRobot import ubot2
from GalaxiaRobot.modules.info import get_chat_info, get_user_info
from GalaxiaRobot.modules.music import download_youtube_audio
from GalaxiaRobot.services.tasks import _get_tasks_text, all_tasks, rm_task
from GalaxiaRobot.services.types import InlineQueryResultCachedDocument
from GalaxiaRobot.utils.functions import test_speedtest
from GalaxiaRobot.utils.pastebin import paste
from GalaxiaRobot.utils.pluginhelper import convert_seconds_to_minutes as time_convert
from GalaxiaRobot.utils.pluginhelper import fetch
    ...
    answerss = [
        InlineQueryResultArticle(
            ...
            input_message_content=InputTextMessageContent(
                "**__ᴄʟɪᴄᴋ ᴀ ʙᴜᴛᴛᴏɴ ᴛᴏ ɢᴇᴛ sᴛᴀʀᴛᴇᴅ.__**"
            ),
...
async def translate_func(answers, lang, tex):
    ...
    msg = f"""
__**ᴛʀᴀɴꜱʟᴀᴛᴇᴅ ғʀᴏᴍ {result.src} ᴛᴏ {result.dest}**__
**ɪɴᴘᴜᴛ:**
{tex}
**ᴏᴜᴛᴘᴜᴛ:**
...
async def urban_func(answers, text):
    ...
    for i in results:
        ...
        msg = f"""
**ϙᴜᴇʀʏ:** {text}
**ᴅᴇғɪɴɪᴛɪᴏɴ:** __{clean(i.definition)}__
...
**ᴍᴀɢɴᴇᴛ:** `{magnet}`"""
        ...
        description = f"{ꜱɪᴢᴇ} | {upload_date} | ꜱᴇᴇᴅꜱ: {seeds}"
        answers.append(
            InlineQueryResultArticle(
                title=title,
...
async def youtube_func(answers, text):
    ...
    for i in results:
        ...
        buttons.add(InlineKeyboardButton("ᴡᴀᴛᴄʜ", url=video_url))
        caption = f"""
**ᴛɪᴛʟᴇ:** {i.title}
**ᴠɪᴇᴡꜱ:** {i.views}
**ᴄʜᴀɴɴᴇʟ:** {i.channel}
...
async def music_inline_func(answers, query):
    ...
    try:
        ...
        print(e)
        msg = f"ʏᴏᴜ ɴᴇᴇᴅ ᴛᴏ ᴊᴏɪɴ ʜᴇʀᴇ ᴡɪᴛʜ ʏᴏᴜʀ ʙᴏᴛ ᴀɴᴅ ᴜsᴇʀʙᴏᴛ ᴛᴏ ɢᴇᴛ ᴄᴀᴄʜᴇᴅ ᴍᴜsɪᴄ.\n{group_invite}"
        answers.append(
            InlineQueryResultArticle(
                title="ERROR",
                description="ᴄʟɪᴄᴋ ʜᴇʀᴇ ᴛᴏ ᴋɴᴏᴡ ᴍᴏʀᴇ.",
                input_message_content=InputTextMessageContent(
                    msg, disable_web_page_preview=True
                ),
            )
...
async def speedtest_init(query):
    ...
    if user_id not in DEV_USERS:
        ...
        return answers
    msg = "**ᴄʟɪᴄᴋ ᴛʜᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ ᴛᴏ ᴘᴇʀғᴏʀᴍ ᴀ sᴘᴇᴇᴅᴛᴇsᴛ**"
    button = InlineKeyboard(row_width=1)
    button.add(InlineKeyboardButton(text="ᴛᴇsᴛ", callback_data="test_speedtest"))
    answers.append(
        InlineQueryResultArticle(
            title="ᴄʟɪᴄᴋ ʜᴇʀᴇ",
            input_message_content=InputTextMessageContent(msg),
...
async def test_speedtest_cq(_, cq):
    if cq.from_user.id not in DEV_USERS:
        return await cq.answer("ᴛʜɪs ɪsɴ'ᴛ ғᴏʀ ʏᴏᴜ!")
    inline_message_id = cq.inline_message_id
    await app.edit_inline_text(inline_message_id, "**ᴛᴇsᴛɪɴɢ**")
    loop = asyncio.get_running_loop()
    download, upload, info = await loop.run_in_executor(None, test_speedtest)
    msg = f"""
**ᴅᴏᴡɴʟᴏᴀᴅ:** `{download}`
**ᴜᴘʟᴏᴀᴅ:** `{upload}`
**ʟᴀᴛᴇɴᴄʏ:** `{info['latency']} ms`
**ᴄᴏᴜɴᴛʀʏ:** `{info['country']} [{info['cc']}]`
**ʟᴀᴛɪᴛᴜᴅᴇ:** `{info['lat']}`
**ʟᴏɴɢɪᴛᴜᴅᴇ:** `{info['lon']}`
"""
...
**{result.title}**
**ᴛʏᴘᴇ:** {result.type}
**ʀᴀᴛɪɴɢ:** {result.rating}
**ɢᴇɴʀᴇ:** {genre}
**ʀᴇʟᴇᴀsᴇ ᴅᴀᴛᴇ:** {result.releaseDate}
**ᴅᴇsᴄʀɪᴘᴛɪᴏɴ:** __{description}__
"""
        buttons = InlineKeyboard(row_width=1)
        buttons.add(
            InlineKeyboardButton(
...
async def execute_code(query):
    ...
    if len(text.split()) == 1:
        ...
        if not response.ok:
            ...
            out = "sᴛᴅᴏᴜᴛ" if stdout else ("STDERR" if stderr else "No output")
...
async def cancel_task_button(_, query: CallbackQuery):
    ...
    if user_id not in DEV_USERS:
        return await query.answer("ᴛʜɪs ɪs ɴᴏᴛ ғᴏʀ ʏᴏᴜ.")
