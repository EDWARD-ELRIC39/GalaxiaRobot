...
from GalaxiaRobot import aiohttpsession as aiosession
from GalaxiaRobot.ex_plugins.dbfunctions import start_restart_stage
from GalaxiaRobot.utils.http import get, post


async def restart(m: Message):
    if m:
        await start_restart_stage(m.chat.id, m.message_id)
    execvp(executable, [executable, "-m", "GalaxiaRobot"])
...
def generate_captcha():
    ...
    height = 100
    correct_answer = ""
    font = ImageFont.truetype("GalaxiaRobot/resources/default.ttf", 55)
    file = f"GalaxiaRobot/resources/{randint(1000, 9999)}.jpg"
