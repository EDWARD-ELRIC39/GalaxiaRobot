...
from GalaxiaRobot import OWNER_ID, pbot
...
async def evaluate(client, message):
    ...
    evaluation = ""
    if exc:
        evaluation = f"<b>ᴇxᴄᴇᴘᴛɪᴏɴ:</b>\n<code>{html.escape(exc)}</code>\n"
    if stderr:
        evaluation += f"<b>ꜱᴛᴅᴇʀʀ:</b>\n<code>{html.escape(stderr)}</code>\n"
    if stdout:
        evaluation += f"<b>ꜱᴛᴅᴏᴜᴛ:</b>\n<code>{html.escape(stdout)}</code>\n"
    if not evaluation:
        evaluation = "ꜱᴜᴄᴄᴇꜱꜱ ʙᴜᴛ ɴᴏ ᴏᴜᴛᴘᴜᴛ!"
...
async def terminal(client, message):
    if len(message.text.split()) == 1:
        await message.reply("ᴜꜱᴀɢᴇ: `/term echo owo`")
        return
...
""".format(
    ...
    if output:
        if len(output) > 4096:
            with open("GalaxiaRobot/output.txt", "w+") as file:
                file.write(output)
