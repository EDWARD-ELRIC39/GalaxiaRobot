...
from GalaxiaRobot import TEMP_DOWNLOAD_DIRECTORY
...
async def _(event):
    ...
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
                "ʜᴇʏ ʙᴀʙʏ , ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ. ʏᴏᴜ ᴄᴀɴ'ᴛ ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ, ʙᴜᴛ ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ɪɴ ᴍʏ ᴘᴍ 🙂"
            )
            return

    mone = await event.reply("⏳️")
...
async def _(event):
    ...
    if not event.is_reply:
        await event.reply("ʜᴀʏ ʙᴀʙʏ!  ʀᴇᴘʟʏ ᴛᴏ ᴀ ᴢɪᴘ ғɪʟᴇ 🤐")
        return
    if event.is_group:
        if not (await is_register_admin(event.input_chat, event.message.sender_id)):
            await event.reply(
