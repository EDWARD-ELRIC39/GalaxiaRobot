...
from GalaxiaRobot import dispatcher, telethn
from GalaxiaRobot.__main__ import (
    CHAT_SETTINGS,
    DATA_EXPORT,
    DATA_IMPORT,
    HELPABLE,
...
from GalaxiaRobot.modules.helper_funcs.chat_status import dev_plus, sudo_plus
...
def load(update: Update, context: CallbackContext):
    ...
    try:
        imported_module = importlib.import_module("GalaxiaRobot.modules." + text)
    except:
    ...
    load_messasge.edit_text(
        "ꜱᴜᴄᴄᴇꜱꜱғᴜʟʟʏ ʟᴏᴀᴅᴇᴅ ᴍᴏᴅᴜʟᴇ : <b>{}</b>".format(text),
        parse_mode=ParseMode.HTML,
    )
...
def unload(update: Update, context: CallbackContext):
    ...
    if imported_module.__mod_name__.lower() in IMPORTED:
        ...
        unload_messasge.edit_text("ᴄᴀɴ'ᴛ ᴜɴʟᴏᴀᴅ ꜱᴏᴍᴇᴛʜɪɴɢ ᴛʜᴀᴛ ɪꜱɴ ʟᴏᴀᴅᴇᴅ.")
        return
    if "__handlers__" in dir(imported_module):
        handlers = imported_module.__handlers__
        for handler in handlers:
    ...
    unload_messasge.edit_text(
        f"ꜱᴜᴄᴄᴇꜱꜱғᴜʟʟʏ ᴜɴʟᴏᴀᴅᴇᴅ ᴍᴏᴅᴜʟᴇ : <b>{text}</b>",
        parse_mode=ParseMode.HTML,
    )
...
def listmodules(update: Update, context: CallbackContext):
    ...
    for helpable_module in HELPABLE:
        ...
        file_name = file_info.__name__.rsplit("GalaxiaRobot.modules.", 1)[1]
        mod_name = file_info.__mod_name__
        module_list.append(f"- <code>{mod_name} ({file_name})</code>\n")
    module_list = "ғᴏʟʟᴏᴡɪɴɢ ᴍᴏᴅᴜʟᴇꜱ ᴀʀᴇ ʟᴏᴀᴅᴇᴅ : \n\n" + "".join(module_list)
