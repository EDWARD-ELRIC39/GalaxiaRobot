...
from GalaxiaRobot.events import register as tomori
...
async def whatanime(e):
    ...
    if not media:
        await e.reply("`ᴍᴇᴅɪᴀ ʀᴇϙᴜɪʀᴇᴅ`")
        return
    ig = is_gif(media) or is_video(media)
    if not is_image(media) and not ig:
        await e.reply("`ᴍᴇᴅɪᴀ ᴍᴜsᴛ ʙᴇ ᴀɴ ɪᴍᴀɢᴇ ᴏʀ ɢɪғ ᴏʀ ᴠɪᴅᴇᴏ`")
        return
    filename = "file.jpg"
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if not js0:
            await cut.edit("`ɴᴏ ʀᴇsᴜʟᴛs ғᴏᴜɴᴅ.`")
            return
        js0 = js0[0]
