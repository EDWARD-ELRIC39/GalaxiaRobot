...
from GalaxiaRobot import DEV_USERS
from GalaxiaRobot import pbot as app
from GalaxiaRobot.services.sections import section
...
async def get_user_info(user, already=False):
    ...
    caption = section("ᴜꜱᴇʀ ɪɴғᴏ", body)
...
async def get_chat_info(chat, already=False):
    ...
    caption = section("ᴄʜᴀᴛ ɪɴғᴏ", body)
    return [caption, photo_id]
...
async def chat_info_func(_, message: Message):
    try:
        if len(message.command) > 2:
            return await message.reply_text("**ᴜꜱᴀɢᴇ:**ᴄɪɴғᴏ <chat id/username>")
