...
from GalaxiaRobot.utils.pluginhelper import runcmd
    ...
    if not (
        ...
        try:
            ...
            lel = await borg.send_message(
                event.chat_id,
                "ᴅᴏᴡɴʟᴏᴀᴅᴇᴅ ᴛᴏ `{}` sᴜᴄᴄᴇssғᴜʟʟʏ.".format(downloaded_file_name),
            )
            await lel.delete
    if not os.path.exists(downloaded_file_name):
        lel = await borg.send_message(event.chat_id, "ᴅᴏᴡɴʟᴏᴀᴅ ᴜɴsᴜᴄᴇssғᴜʟʟ :(")
        await lel.delete
        return
